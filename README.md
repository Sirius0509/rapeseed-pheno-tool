# 油菜表型拍照测量工具

一个面向油菜单株照片的 H5/PWA 测量工具，支持手机和电脑浏览器使用。

## 功能

- 上传图片或手机拍照
- 标尺校准
- 两点测量株高、主花序高、主花序长度
- 三点测量分支角
- 点击计数一级分枝
- 保存样品信息和测量结果
- 导出 Excel
- 导出带标注图片
- 支持 PWA 添加到手机主屏幕
- 独立角果/籽粒测量模块
- 调用手机、电脑或平板摄像头拍照
- 角果长度测量
- 训练好的 YOLO 模型优先生成籽粒候选点；没有模型时使用 OpenCV 多算法融合生成候选点
- 角果和籽粒数据单独导出 Excel
- 保存原图、自动点位、人工校正点位和删除点位，用于训练数据回流
- 一键导出 YOLO 检测数据集 zip，包括 images、labels、data.yaml 和 metadata.json
- 通过本地 Python 后端一键生成 YOLO 数据集并启动训练任务

## 本地运行

```bash
npm install
npm run dev
```

## 构建

```bash
npm run build
npm run preview
```

## GitHub Pages 部署

仓库推送到 GitHub 后，Actions 会自动构建并发布 `dist`。在仓库设置中启用 Pages，Source 选择 **GitHub Actions**。

## 使用建议

- 每张照片优先对应一株样品。
- 植株和标尺尽量在同一平面。
- 标尺真实长度要固定并清楚记录。
- 分支计数建议只数一级分枝，点在分枝与主茎连接位置。

## 角果/籽粒模块建议

- 每张照片优先对应一个角果或一组已剥离籽粒。
- 建议使用白色或浅蓝色纯色背景、柔光、垂直俯拍，减少培养皿反光。
- 籽粒需要尽量分散，避免重叠和粘连。
- 拍照时镜头尽量垂直向下，光照均匀。
- 候选点结果必须人工确认，可用“补籽粒”和“删籽粒”修正。系统会提示置信度和疑似粘连/异常区域。
- 籽粒自动识别参数会自动保存到本机；分散籽粒、籽粒颜色比背景暗时，建议先点“推荐参数”，系统会按当前照片或框选区域推荐阈值，再生成候选点。
- 传统图像识别无法稳定拆开所有粘连籽粒；粘连或重叠时仍需要用“补籽粒/删籽粒”快速人工校正，校正点位会进入训练数据。
- 角果长度和籽粒数支持两阶段拍照：先拍角果确认长度并保存长度照片，再剥开同一角果重新拍籽粒照片，最后保存为同一条记录。
- 测完角果长度后可点击“确认角果长度”自动填入，也可以手动输入保存长度；生成并校正候选点后可点击“确认籽粒数”自动填入，也可以手动输入籽粒数。保存记录会使用最终编辑后的数值。
- 结果表支持单条记录编辑和删除，误触保存或重复记录可以直接在结果区修正。
- 手机端将确认长度、确认籽粒数和保存放在照片下方；自动识别参数、训练数据和结果表支持折叠，减少滚动。
- 样品编号支持输入编号示范后生成下拉选择，重复数提供 0-9 选择。
- 角果/籽粒 Excel 会包含明细、按样品编号汇总平均值、按样品编号+重复汇总平均值。
- 角果/籽粒记录使用 IndexedDB 保存完整图片和点位，避免大量照片超过 localStorage 容量后刷新丢失。
- 填写电脑后端地址后，手机端保存记录会同步到电脑后端；电脑端可从后端同步记录，也可以直接用后端共享记录一键训练模型。
- 支持 Supabase 云同步设置：同一个 Supabase URL、anon key 和 `rapeseed-images` bucket 可让手机和电脑共享云端记录。
- 独立“胎萌测定”模块支持标尺校准、种子区域框选、ImageJ 风格突出检测、逐粒人工校正、胎萌率统计和单独 Excel 导出。
- 电脑后端支持直接拉取当前登录用户的 Supabase 云端记录和图片训练 YOLO 模型，不需要先手动把云端数据同步到浏览器本地。

## Supabase 配置

建好 `silique_records` 表和 `rapeseed-images` bucket 后，还需要允许 anon 访问 Storage。SQL Editor 中执行：

如果要多人账号隔离，先给记录表增加 `user_id`，并把旧的 anon 全开放策略替换为当前用户策略：

```sql
alter table silique_records
add column if not exists user_id uuid references auth.users(id) on delete cascade;

drop policy if exists "allow anon read silique records" on silique_records;
drop policy if exists "allow anon insert silique records" on silique_records;
drop policy if exists "allow anon update silique records" on silique_records;
drop policy if exists "allow anon delete silique records" on silique_records;

create policy "users can read own silique records"
on silique_records
for select
to authenticated
using (auth.uid() = user_id);

create policy "users can insert own silique records"
on silique_records
for insert
to authenticated
with check (auth.uid() = user_id);

create policy "users can update own silique records"
on silique_records
for update
to authenticated
using (auth.uid() = user_id)
with check (auth.uid() = user_id);

create policy "users can delete own silique records"
on silique_records
for delete
to authenticated
using (auth.uid() = user_id);
```

Storage 建议也按用户目录隔离：

```sql
drop policy if exists "allow anon read rapeseed images" on storage.objects;
drop policy if exists "allow anon upload rapeseed images" on storage.objects;
drop policy if exists "allow anon update rapeseed images" on storage.objects;
drop policy if exists "allow anon delete rapeseed images" on storage.objects;

create policy "users can read own rapeseed images"
on storage.objects
for select
to authenticated
using (bucket_id = 'rapeseed-images' and auth.uid()::text = (storage.foldername(name))[1]);

create policy "users can upload own rapeseed images"
on storage.objects
for insert
to authenticated
with check (bucket_id = 'rapeseed-images' and auth.uid()::text = (storage.foldername(name))[1]);

create policy "users can update own rapeseed images"
on storage.objects
for update
to authenticated
using (bucket_id = 'rapeseed-images' and auth.uid()::text = (storage.foldername(name))[1])
with check (bucket_id = 'rapeseed-images' and auth.uid()::text = (storage.foldername(name))[1]);

create policy "users can delete own rapeseed images"
on storage.objects
for delete
to authenticated
using (bucket_id = 'rapeseed-images' and auth.uid()::text = (storage.foldername(name))[1]);
```

## 云端存储

当前版本已预留“云端链接”字段，但还没有接入具体云服务。推荐后续接入 Supabase Storage + Database：

- Storage 保存角果和籽粒照片。
- Database 保存样品编号、角果编号、长度、籽粒数、照片 URL、测量日期等元数据。
- 接入后可以实现多人共享、自动分类归档和跨设备查询。

## 训练数据回流

角果/籽粒模块保存记录时会同步保存：

- 原图 data URL
- 自动识别点位
- 人工新增/删除后的最终点位
- 删除点位
- 图片质量标签
- 样品编号、角果编号、基因型、重复和备注

人工校正应通过“补籽粒”和“删籽粒”修改点位，而不是只修改数字。这样每次校正都会沉淀为可训练的检测标注。

“导出 YOLO 数据集”会把最终校正点位转换为固定大小 bbox，自动生成：

```text
dataset_xxx/
├── images/train/
├── images/val/
├── labels/train/
├── labels/val/
├── data.yaml
└── metadata.json
```

质量标签为“不进训练集”的记录不会进入 YOLO 导出。

## 模型建议

整株模块和角果/籽粒模块都支持导出 JSON 训练数据清单。

建议模型拆分：

- 整株表型：使用关键点检测模型，例如 YOLO-pose 或 HRNet，识别标尺端点、基部、最高点、主花序端点和分枝关键点，再用几何公式计算表型。
- 角果长度：优先使用传统图像处理；背景复杂后再考虑 YOLO-seg 或 Mask R-CNN。
- 籽粒计数：建议单独使用目标检测或实例分割模型，例如 YOLO 检测模型、Cellpose/StarDist 风格模型。它不建议和整株关键点模型共用。

一个统一系统可以管理所有数据，但模型本身建议按任务拆分。

## 籽粒识别服务

`backend/` 目录提供一个可选 Python FastAPI 服务。候选点生成逻辑为：

1. 如果 `backend/runs/*/train/seed_detector/weights/best.pt` 存在，优先使用训练好的 YOLO 检测模型。
2. 如果还没有训练模型，使用 OpenCV 多算法融合：暗色/亮色自动判断、光照校正、阈值分割、分水岭、SimpleBlobDetector 和 Hough circle 候选合并。

后端同时提供本地 YOLO 训练任务接口。

服务会返回候选点数量、置信度、疑似粘连区域数量和复核提示。大面积连通区域会按当前图片的单粒中位面积估算可能包含的籽粒数，但仍建议人工确认。

本地运行：

```bash
cd backend
python3 -m venv .venv
source .venv/bin/activate
pip install -r requirements.txt
uvicorn app:app --host 0.0.0.0 --port 8000
```

前端“角果/籽粒”模块中可填写识别服务地址：

```text
http://127.0.0.1:8000
```

正式线上使用时，后端需要部署为 HTTPS，例如 Render、Railway、Fly.io、阿里云、腾讯云或实验室服务器反向代理。

## 本地训练模型

训练入口在“角果/籽粒”模块的“训练数据”面板。使用前需要：

1. 运行 `backend/` 后端。
2. 在“识别服务地址”填写 `http://127.0.0.1:8000`。
3. 保存至少 2 张有原图、有最终籽粒点位、质量不是“不进训练集”的记录。
4. 点击“开始训练模型”。

后端会在 `backend/runs/` 下生成数据集和训练结果。默认使用 `yolo11n.pt`、`imgsz=1024`、`epochs=50`、`batch=4`。训练完成后面板会显示最佳模型路径。

注意：GitHub Pages 不能直接训练模型；训练必须依赖本地或云端 Python 后端。第一次训练时 ultralytics 会下载预训练权重，需要联网。
